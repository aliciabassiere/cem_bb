...
parser.add_argument("--name", type=str, default="default_simulation", help="Name of the simulation batch")
parser.add_argument("--nombres_simu", type=int, default=64, help="Number of simulations")
parser.add_argument("--coal_phase_out", type=str, choices=["y", "n"], default="n", help="Apply Coal Phase-out Plan? [y/n]")
parser.add_argument("--carbon_tax", type=str, choices=["y", "n"], default="n", help="Apply carbon tax in 2030? [y/n]")
parser.add_argument("--lambda_weight", type=float, default=0, help="Carbon target constraining parameter")
parser.add_argument("--mu_weight", type=float, default=0, help="Gas investing constraining parameter")
parser.add_argument("--kappa_weight", type=float, default=0, help="Wind investing constraining parameter")
parser.add_argument("--nu_weight", type=float, default=0, help="Solar investing constraining parameter")

parser.add_argument("--kwbound", type=int, default=70000, help="Maximum capacity for wind energy")
parser.add_argument("--kgbound", type=int, default=60000, help="Maximum capacity for gas energy")
parser.add_argument("--ksbound", type=int, default=86600, help="Maximum capacity for solar energy")
parser.add_argument("--step_w", type=int, default=5000, help="Discretization size for wind energy")
parser.add_argument("--step_s", type=int, default=5000, help="Discretization size for solar energy")
parser.add_argument("--step_g", type=int, default=500, help="Discretization size for gas energy")

args = parser.parse_args()

name = args.name
nombres_simu = args.nombres_simu
coal_phase_out = args.coal_phase_out
carbon_tax = args.carbon_tax
lambda_weight = args.lambda_weight
mu_weight = args.mu_weight
kappa_weight = args.kappa_weight
nu_weight = args.nu_weight

kwbound = args.kwbound
kgbound = args.kgbound
ksbound = args.ksbound
step_w = args.step_w
step_s = args.step_s
step_g = args.step_g
...
class SimulationParameters:
    ...
    def __init__(self):
        ...
        if self.coal_phase_out.lower() == "y":
            self.kc = [30000, 27300, 25800, 23900, 21800, 20200, 17300, 16800, 14900, 12700, 10600, 8700, 8100,
...
class Scenario:
    ...
    def __init__(self):
        ...
        scenarios_csv_path = os.path.join(simu_parameters.path2, "scenarios.csv")
...
class TechnoParameters:
    ...
    def __init__(self):
        self.kwbound = kwbound
        self.kgbound = kgbound
        self.ksbound = ksbound
        self.step_w = step_w
        self.step_s = step_s
        self.step_g = step_g
        self.unit_w = 1000
