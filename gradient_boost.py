...
class GradientBoostingModel:
    ...
    def minimize_quantity(self, kw_initial, ks_initial, kg_initial, t, grid_minimum, initial_value=0):

        def objective_function(params):
            kw_obj, ks_obj, kg_obj = params
            value = self.predicted_value_deterministic(kw_obj, ks_obj, kg_obj)
            invest = investment_functions.invest(kw_obj-kw_initial, ks_obj-ks_initial, kg_obj-kg_initial, t)
    ...
    def minimize_expected_quantity(self, kw_initial, ks_initial, kg_initial, D, t, grid_minimum, initial_value=0):
        ...
        def objective_function(params):
            kw, kpv, kg = params
            D_iter = D if hasattr(D, '__iter__') and not isinstance(D, str) else [D]
            mean_value = np.mean([self.predicted_value(kw, kpv, kg, d) for d in D_iter])
