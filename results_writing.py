...
def save_results_to_csv(name, optimal_trajectory, number_simulations=simu_parameters.n_simu, column_names=None, offset=0):
    ...
    if path_results.endswith("_0"):
        path_results = path_results[:-2]
    if not os.path.exists(path_results):
        os.makedirs(path_results)

    costs_df.columns = column_names
    carbon_emissions_df.columns = column_names
    adequacy_df.columns = column_names
    adequacy_hourly_df.columns = column_names
    quantity_pv_df.columns = column_names
    quantity_wind_df.columns = column_names
    quantity_gas_df.columns = column_names
    quantity_coal_df.columns = column_names
    quantity_peak_df.columns = column_names
    quantity_pv_hourly_df.columns = column_names
    quantity_wind_hourly_df.columns = column_names
    quantity_gas_hourly_df.columns = column_names
    quantity_coal_hourly_df.columns = column_names
    quantity_peak_hourly_df.columns = column_names
    defaults_df.columns = column_names
    cost_pv_df.columns = column_names
    cost_wind_df.columns = column_names
    cost_gas_df.columns = column_names
    cost_coal_df.columns = column_names
    cost_peak_df.columns = column_names
    cost_default_df.columns = column_names
    marginal_cost_df.columns = column_names
    load_df.columns = column_names
    load_hourly_df.columns = column_names
    inv_cost_df.columns = column_names
    mu_constraint_df.columns = column_names
    nu_constraint_df.columns = column_names
    kappa_constraint_df.columns = column_names
    lambda_constraint_df.columns = column_names
    npv_df.columns = column_names
    npv_carbon_free_df.columns = column_names
    npv_constrained_df.columns = column_names
    lambda_discounted_df.columns = column_names
    kappa_discounted_df.columns = column_names
    nu_discounted_df.columns = column_names
    cost_pv_discounted_df.columns = column_names
    cost_wind_discounted_df.columns = column_names
    cost_gas_discounted_df.columns = column_names
    cost_coal_discounted_df.columns = column_names
    cost_peak_discounted_df.columns = column_names
    cost_default_discounted_df.columns = column_names
    inv_cost_discounted_df.columns = column_names
